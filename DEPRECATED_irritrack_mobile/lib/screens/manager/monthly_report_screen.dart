...
class _MonthlyReportScreenState extends State<MonthlyReportScreen> {
  ...
  String _generateCSVReport(Map<String, dynamic> reportData) {
    ...
    // Header
    buffer.writeln('$companyName Monthly Report');
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final companyName = widget.authService.storage.companySettings?.companyName ?? 'IrriTrack';
      final monthStr = DateFormat('yyyy-MM').format(selectedMonth);
      final filename = '${companyName.replaceAll(' ', '_')}_Report_$monthStr.pdf';

      await PdfService.previewAndPrint(pdfBytes, filename);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
